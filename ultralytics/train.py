...
# 모델을 로드하세요..
model = YOLO('yolo11s-seg.yaml')  # YAML에서 새 모델 구축
model = YOLO('yolo11s-seg.pt')  # 사전 훈련된 모델 로드 (훈련을 위해 권장됨)
...
if __name__ == '__main__':
    ...
    results = model.train(data="datasets/"+dataset_name+"/data.yaml", dropout=0.02, epochs=100, imgsz=640, workers=8, batch=8, patience=50)
