        ...
        try:
                    ...
                    if grabResults[i].GrabSucceeded():
                        ...
                        if (int(results[i][0].boxes.cls[d_num]) == 1):
                            ...
                            if int(results[i][0].boxes.cls[d_num]) == 1 & (time1 - time2 > 500000):# and (current_cnt > mem_cnt):
                                ...
                                cv2.imwrite('C:/image/'+detected_date+'/box/'+ str(s_n1) + '_'  + str(d_meter) + '_'  + str(err_cnt_array) + '_'  + str(i) + '_' + detected_time+'.jpg', results[i][0].plot())

                                for l in range(len(cameras)):
                                    cv2.imwrite('C:/image/'+detected_date+'/Original/'+ str(s_n1) + '_'  + str(d_meter) + '_'  + str(err_cnt_array) + '_'  + str(l) + '_' + detected_time+'.jpg', images[l])
                                    
                                    # s_time(제품 키값), material_number(제품번호), seq2(몇번쨰 생성), d_meter(몇미터에서 생성), type(오류 유형), d_time(감지 시간), image(이미지 위치), area(면적)

                                # 이미지 저장 위치
                                image = "C:/image/"+detected_date+"/box/"+ str(s_n1) + '_'  + str(d_meter) + '_'  + str(err_cnt_array) + '_'  + str(i) + '_' + str(detected_time)+".jpg"

                                detect.write_sql(mmddhhnnss, str(s_n1), err_cnt_array, d_meter, type, detected_time, image, area)
                                    # time.sleep(1)
                        ...
                        if (not (cable_area_base == 0)) and (int(np.mean(masks)) > cable_area_base*1.30) and (len(cameras)==i+1):
                            ...
                            if (time3-time4 > 500000):# and (current_cnt > mem_cnt):
                                ...
                                cv2.imwrite('C:/image/'+detected_date+'/area_box/'+ str(s_n1) + '_'  + str(d_meter) + '_'  + str(err_cnt_array) + '_'  + str(i) + '_' + detected_time+'.jpg', results[i][0].plot())
                                
                                for l in range(len(cameras)):
                                    print(l)
                                    cv2.imwrite('C:/image/'+detected_date+'/area_Original/'+ str(s_n1) + '_'  + str(d_meter) + '_'  + str(err_cnt_array) + '_'  + str(l)+ '_' + detected_time+'.jpg', images[l])
                                    
                                    # 이미지 저장 위치
                                image = "C:/image/"+detected_date+"/area_box/"+ str(s_n1) + '_'  + str(d_meter) + '_'  + str(err_cnt_array) + '_'  + str(i) + '_' + str(detected_time)+".jpg"
            ...
            if len(mean_masks) >= 10:
                ...
                if(time5-time6 > 1):# and (current_cnt > mem_cnt):
                    ...
                    mem_meter = current_meter
                    mem_cnt = current_cnt
                    areadb.write_sql(mmddhhnnss, str(s_n1), int(np.mean(mean_masks)))
